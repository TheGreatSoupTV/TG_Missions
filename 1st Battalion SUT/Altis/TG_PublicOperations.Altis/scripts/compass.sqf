/*
  fn:       voyagerCompass.sqf
  author:   deYoonge and Flax
  description:
        Sets up compass for HUD must be called via '[] execVM'
*/

scriptName "fn_voyagerCompass.sqf";
...
34 cutRsc ["RscTitleDisplayEmpty", "PLAIN"];

player addAction ["Compass", {
disableSerialization;
...
nc_dialog_controls = [
  ...
  ["(uiNamespace getVariable 'nc_dialogDisplay') closeDisplay 2"]],
	...
	[[0,1, nc_x], "folder set [0, (_this select 1)]; nc_barOffsetX = ((_this select 1) * safezoneH + safezoneY)"]],
	...
	[[0,1, nc_y], "folder set [1, (_this select 1)]; nc_barOffsetY = ((_this select 1) * safezoneH + safezoneY)"]],
	...
	[[0,5, nc_w], "folder set [2, (_this select 1)]; nc_w = (_this select 1)"]],
	...
	[[1,3, nc_span], "folder set [3, (_this select 1)]; nc_span = (_this select 1)"]],
	...
	[[0.8,1.4, nc_fontSize], "folder set [5, (_this select 1)]; nc_fontSize = (_this select 1); _comp = [nc_type, nc_needle, nc_fontSize, nc_shadow, nc_fontOption, nc_alpha, nc_colorOption] call fnc_loadCompass"]],
  ...
  [1005,"nc_color_code",[1,"",[0.326719 * safezoneW + safezoneX,0.621 * safezoneH + safezoneY,0.04125 * safezoneW,0.022 * safezoneH],[1,1,1,1],[0.03,0.03,0.03,1],[-1,-1,-1,-1],"","RscEdit"],[]],
	...
	[[-2,2, nc_yM], "folder set [12, (_this select 1)]; nc_marker_offset = (_this select 1) + nc_barOffsetY"]],
